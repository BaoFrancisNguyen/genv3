...
Note: Ce fichier remplace l'ancien src/core/exporter.py pour une cohérence
de nommage avec les imports dans les services.
"""
    ...
    def _export_format(
        ...
        try:
            ...
            if format_type == 'csv':
                ...
                format_results['total_size_mb'] = format_results['files']['excel']['size_mb']
...
def optimize_dataframe_for_export(df: pd.DataFrame, data_type: str) -> pd.DataFrame:
    """
    Optimise un DataFrame pour l'export
    ...
    # Conversion des colonnes object en category si approprié
    ...
    if data_type == 'buildings':
        ...
        if 'latitude' in df_optimized.columns:
            df_optimized['latitude'] = df_optimized['latitude'].round(6)
        if 'longitude' in df_optimized.columns:
            df_optimized['longitude'] = df_optimized['longitude'].round(6)
        
        # Arrondir les surfaces
        if 'surface_area_m2' in df_optimized.columns:
            df_optimized['surface_area_m2'] = df_optimized['surface_area_m2'].round(1)
        ...
        if 'consumption_kwh' in df_optimized.columns:
            df_optimized['consumption_kwh'] = df_optimized['consumption_kwh'].round(4)
        
        # Arrondir température
        if 'temperature_c' in df_optimized.columns:
            df_optimized['temperature_c'] = df_optimized['temperature_c'].round(2)
        
        # Optimiser les booléens
...
def create_export_summary(export_results: Dict) -> str:
    """
    Crée un résumé textuel de l'export
    ...
    Returns:
        str: Résumé formaté
    """
    if not export_results.get('success', False):
        return "❌ Export échoué"
    
    files_created = export_results.get('files_created', {})
    total_size = export_results.get('total_size_mb', 0)
    
    summary_lines = [
        "✅ Export terminé avec succès",
        f"📊 {len(files_created)} formats créés",
        f"💾 Taille totale: {total_size:.2f} MB"
    ]
    
    # Détails par format
    for format_type, format_data in files_created.items():
        if format_data.get('success', True):
            files_count = len(format_data.get('files', {}))
            format_size = format_data.get('total_size_mb', 0)
            summary_lines.append(f"  - {format_type.upper()}: {files_count} fichiers ({format_size:.2f} MB)")
    
    # Statistiques de validation
    validation = export_results.get('validation_results', {})
    if validation.get('warnings'):
        summary_lines.append(f"⚠️ {len(validation['warnings'])} avertissements")
    
    return "\n".join(summary_lines)


def batch_export_multiple_datasets(
    datasets: List[Dict], 
    export_directory: str = None,
    formats: List[str] = ['csv', 'parquet']
) -> Dict:
    """
    Exporte plusieurs datasets en lot
    
    Args:
        datasets: Liste de dictionnaires avec 'buildings_df', 'timeseries_df', 'name'
        export_directory: Dossier d'export
        formats: Formats à exporter
        
    Returns:
        Dict: Résultats d'export en lot
    """
    exporter = DataExporter(export_directory)
    ...
    for i, dataset in enumerate(datasets):
        try:
            ...
            buildings_df = dataset.get('buildings_df', pd.DataFrame())
            timeseries_df = dataset.get('timeseries_df', pd.DataFrame())
            
            logger.info(f"🔄 Export dataset {i+1}/{len(datasets)}: {dataset_name}")
            
            result = exporter.export_complete_dataset(
            ...
            batch_results['datasets_processed'] += 1
            batch_results['total_size_mb'] += result['total_size_mb']
            
        except Exception as e:
            error_msg = f"Dataset {dataset.get('name', i)}: {str(e)}"
            batch_results['errors'].append(error_msg)
            logger.error(f"❌ {error_msg}")
    
    # Succès global si au moins un dataset exporté
    batch_results['success'] = batch_results['datasets_processed'] > 0
    
    return batch_results


def compress_export_files(export_directory: str, archive_name: str = None) -> str:
    """
    Compresse tous les fichiers d'export en archive ZIP
    
    Args:
        export_directory: Dossier contenant les fichiers
    ...
    if not archive_name:
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        archive_name = f"malaysia_electricity_export_{timestamp}.zip"
    
    archive_path = os.path.join(export_directory, archive_name)
    
    with zipfile.ZipFile(archive_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        for root, dirs, files in os.walk(export_directory):
            for file in files:
                if file != archive_name:  # Ne pas inclure l'archive elle-même
                    file_path = os.path.join(root, file)
                    arcname = os.path.relpath(file_path, export_directory)
                    zipf.write(file_path, arcname)
    
    logger.info(f"📦 Archive créée: {archive_name}")
    return archive_path


def cleanup_old_exports(export_directory: str, max_age_days: int = 7) -> int:
    """
        ...
        if os.path.isfile(filepath):
            ...
            if file_time < cutoff_time:
                try:
                    ...
                    logger.warning(f"⚠️ Impossible de supprimer {filename}: {str(e)}")
...
if __name__ == '__main__':
    ...
    estimates = calculate_file_size_estimate(1000, 50000)
    print(f"\n📊 Estimations taille:")
    for format_name, estimate in estimates.items():
        print(f"  - {format_name}: {estimate['size_formatted']}")
    
    # Validation dossier
    validation = validate_export_directory(exporter.export_directory)
    print(f"\n✅ Dossier export valide: {validation['valid']}")
    
    print("\n🎉 Tests de l'exporteur terminés avec succès")
